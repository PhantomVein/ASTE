...
class BiaffineParser(object):
    def __init__(self, model, root_id):
        self.options_file = "elmo/elmo_2x4096_512_2048cnn_2xhighway_options.json"
        self.weight_file = "elmo/elmo_2x4096_512_2048cnn_2xhighway_weights.hdf5"
        self.embedding_file = './experiments/embedding'
        if os.path.exists(self.embedding_file):
            self.pickle_embedding = pickle.load(open(self.embedding_file, 'rb'))
            print('Embedding file has loaded.')
        else:
            self.pickle_embedding = {}
            print('No existed Embedding file.')
        ...
        if self.use_cuda:
            self.elmo = self.elmo.cuda(self.device)

    def __del__(self):
        pickle.dump(self.pickle_embedding, open(self.embedding_file, 'wb'))
        print('Embedding has saved to file.')

    def elmo_embedding(self, batch_words, masks):
        batch_embedding = []
        is_loaded_flag = False
        while not is_loaded_flag:
            is_loaded_flag = True
            batch_embedding = []
            for words in batch_words:
                i = ' '.join(words)
                if i in self.pickle_embedding:
                    batch_embedding.append(torch.from_numpy(self.pickle_embedding[i]).type(torch.FloatTensor))
                else:
                    is_loaded_flag = False
                    word_ids = batch_to_ids(batch_words)
                    if self.use_cuda:
                        word_ids = word_ids.cuda(self.device)
                    hidden = self.elmo(word_ids)
                    embedding = hidden['elmo_representations'][1].data.cpu().numpy()
                    for k, v, mask in zip(batch_words, embedding, masks):
                        l = torch.sum(mask).item()
                        self.pickle_embedding[' '.join(k)] = v[:int(l), :]
                    break
        batch_embedding = torch.nn.utils.rnn.pad_sequence(batch_embedding, batch_first=True)
        if self.use_cuda:
            batch_embedding = batch_embedding.cuda(self.device)
        return batch_embedding

    def forward(self, words, tags, masks):
        if self.use_cuda:
            tags = tags.cuda(self.device)
            masks = masks.cuda(self.device)

        embeddings = self.elmo_embedding(words, masks)

        arc_logits, rel_logits = self.model.forward(embeddings, tags, masks)
        # cache
    ...
    def parse(self, words, tags, lengths, masks):
        if words is not None:
            self.forward(words, tags, masks)
        ROOT = self.root
