            ...
            if batch_iter % config.validate_every == 0 or batch_iter == batch_num:
                ...
                if best_F < dev_F:
                    ...
                    if config.save_after > 0 and iter > config.save_after:
                        print("save model to ", config.save_model_path + '.' + str(os.getppid()))
                        torch.save(parser.model.state_dict(), config.save_model_path + '.' + str(os.getppid()))
                        print("save ok.")
...
def evaluate(gold_insts, parser, vocab, outputFile):
    ...
    for onebatch in data_iter(gold_insts, config.test_batch_size, False):
        words, extwords, tags, heads, rels, lengths, masks = batch_data_variable(onebatch, vocab, eval=True)
        count = 0
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--size', default=100)
    ...
    train_data = train_data[:int(args.size)*len(train_data)//100]
    print("data_size: ", args.size, "%")
