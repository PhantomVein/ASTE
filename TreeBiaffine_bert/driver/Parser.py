...
class BiaffineParser(object):
    ...
    def forward(self, bert_indice_input, bert_segments_ids, bert_piece_ids, bert_mask, tags, masks):
        if self.use_cuda:
            bert_indice_input = bert_indice_input.cuda(self.device)
            bert_segments_ids = bert_segments_ids.cuda(self.device)
            bert_piece_ids = bert_piece_ids.cuda(self.device)
            bert_mask = bert_mask.cuda(self.device)
            tags = tags.cuda(self.device)
            masks = masks.cuda(self.device)

        arc_logits, rel_logits = self.model.forward(bert_indice_input, bert_segments_ids, bert_piece_ids, bert_mask, tags, masks)
        # cache
    ...
    def parse(self, bert_indice_input, bert_segments_ids, bert_piece_ids, bert_mask, tags, lengths, masks):
        if bert_indice_input is not None:
            self.forward(bert_indice_input, bert_segments_ids, bert_piece_ids, bert_mask, tags, masks)
        ROOT = self.root
